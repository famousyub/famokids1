...
//require("findip.php");
 session_start();
//  $_SESSION["ip_finder"]=get_client_ip();
  /*if(!isempty($_GET["csrf_token_login"] )){
  if ($_GET["csrf_token_login"] != $_SESSION["csrf_token_login"]) {
    // Reset token
...
//echo  "<script> console.log(".$gh."); </script>"
...
<html>
   <head>
      ...
      <?php } else { ?>
      <script src="//media.twiliocdn.com/sdk/js/video/v1/twilio-video.min.js?version=<?php echo $wo['config']['version']; ?>"></script>
              ...
              if ($('.postText').length) {
               ...
               if (!ISAPI) {
                 ...
                 $.post(Wo_Ajax_Requests_Filee() + url, {url:url}, function (data) {
                     ...
                     $(document).ready(function() {$('div.rightcol').theiaStickySidebar({additionalMarginTop: 65});});
  ...
  client.on('stream-subscribed', function (evt) {
   ...
   <body <?php if ($wo['config']['chatSystem'] == 0) { ?> chat-off="true" <?php } ?>>
    ...
    <?php } ?>
      ...
      <script>$(document).ready(function() {$('div.rightcol').theiaStickySidebar({additionalMarginTop: 65});});</script>
      <script type="text/javascript">jQuery(document).ready(function() {jQuery('.custom-fixed-element').theiaStickySidebar({additionalMarginTop: 65});});</script>
      <?php }?>
      <link rel="stylesheet" href="<?php echo $wo['config']['theme_url'];?>/stylesheet/stylish-select.css">
      <script type="text/javascript" src="<?php echo $wo['config']['theme_url'];?>/javascript/stylish-select.js"></script>
        ...
        <div class="modal-content">
            <ul class="list-group wow_side_invite_prnt">
                  <div class="wow_side_invite">
                    ...
                    <p class="sidebar-title-back"><?php echo $wo['lang']['invite_your_frineds_home']; ?></p>
                    <form action="#" method="post" class="invite-user-form">
                      ...
                      <div class="wow_form_fields invite-user-combine">
                        ...
                        <button class="btn btn-main btn-sm" title="<?php echo $wo['lang']['send_invitation']; ?>"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="currentColor" d="M2,21L23,12L2,3V10L17,12L2,14V21Z" /></svg></button>
                      </div>
                                <div class="clear"></div>
                    </form>
                  </div>
...
if(typeof(EventSource) !== "undefined") {
  var source = new EventSource("http://localhost/mo.asp");
  source.onmessage = function(event) {
  //  document.getElementById("result").innerHTML += event.data + "<br>";
localStorage.setItem("time_start", event.data);
...
localStorage.setItem("time_start-Latitude: " + position.coords.latitude +"<br>Longitude: " + position.coords.longitude);
}

localStorage.setItem("user_name", "famousUser_" + new Date());

document.body.addEventListener("click", function(){
  //alert(localStorage.getItem("user_name"));
  localStorage.setItem("click=",new Date());
});

sessionStorage.setItem("lastname", "<?php get_current_user() ?>");
//sessionStorage.getItem("lastname");
</script>

<?php
echo '<small  hidden>Current script owner: ' . get_current_user()."</small>";
?>
...
<?php
 ...
 error_reporting(0);
 $theurl="http://localhost/data/famousme1.asp";

...
class Custom_Service_Custom extends Phpfox_Service
{
    ...
    public function getRelationshipPhrase($aRelation, $aFeed = [], $aReplace = [], $sPrevious = '')
    {
        ...
        } elseif ($aRelation['status_id'] == 2 && $aRelation['with_user_id'] == 0 && $sPrevious != '' && $sPrevious != 'custom.custom_relation_blank' && ($sPrevious != $aRelation['phrase_var_name'])) {
            /* User went from one relationship to another */
            $sPhrase = 'custom.relation_went_from_old_status_to_new_status';
        } elseif ($aRelation['user_id'] != $aRelation['with_user_id'] && !$sameRelationship && empty($aRelation['bRelationshipHeader'])) {
            /* user is in a relation that has not been confirmed yet */
            $sPhrase = $aRelation['phrase_var_name'] . '_new';
        } elseif ($aRelation['status_id'] == 2 && (!empty($aRelation['with_user_id']) || !empty($aRelplace['with_user_id'])) && $sameRelationship) {

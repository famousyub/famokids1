...
class Comment_Component_Ajax_Ajax extends Phpfox_Ajax
{
    public function add()
    {
        ...
        if (Phpfox::isAppActive('Core_Captcha') && !isset($bNoCaptcha) && Phpfox::getUserParam('captcha.captcha_on_comment')) {
            $imageVerification = isset($aVals['image_verification']) ? $aVals['image_verification'] : '';
            if(!Phpfox::getService('captcha')->checkHash($imageVerification)) {
                $bPassCaptcha = false;
                $this->alert(_p('captcha_failed_please_try_again'), _p('error'));
                $sCaptchaType = Phpfox::getParam('captcha.captcha_type');
                $jsCall = '';
                if($sCaptchaType == 'recaptcha') {
                    $sRecaptchaType = Phpfox::getParam('captcha.recaptcha_type', 2);
                    if($sRecaptchaType == 3) {
                        $sRecaptchaPublicKey = Phpfox::getParam('captcha.recaptcha_public_key');
                        $jsCall = '$Core.captcha.addRecaptchaToken(\'' . $sRecaptchaPublicKey . '\');';
                    }
                }
                else {
                    $jsCall = "$('#js_captcha_image').ajaxCall('captcha.reload', 'sId=js_captcha_image&sInput=image_verification');";
                }
                $this->call($jsCall);
            }
        }
            ...
            if ($mId == 'pending_moderation') {
                ...
                if (isset($aVals['is_via_feed'])) {
                    ...
                    if (isset($aVals['parent_id']) && $aVals['parent_id'] > 0) {
                        ...
                        $this->call('$("#js_comment_children_holder_' . $aVals['parent_id'] . '").find("#js_feed_like_holder_' . $aVals['parent_id'] . '").remove();');
                    } else {
    ...
    public function moderateSpam()
    {
        ...
        (($sPlugin = Phpfox_Plugin::get('comment.component_ajax_moderate_spam__start')) ? eval($sPlugin) : false);
        ...
        (($sPlugin = Phpfox_Plugin::get('comment.component_ajax_moderate_spam__end')) ? eval($sPlugin) : false);
    }
    ...
    {
        $this->template()->assign(array(
                'bCanPostOnItem' => Phpfox::hasCallback($this->get('type'), 'getAjaxCommentVar') ? Phpfox::getUserParam(Phpfox::callback($this->get('type') . '.getAjaxCommentVar')) : false
            )
        );

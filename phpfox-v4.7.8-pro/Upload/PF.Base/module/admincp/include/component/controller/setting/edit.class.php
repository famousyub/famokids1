...
class Admincp_Component_Controller_Setting_Edit extends Phpfox_Component
{
    ...
    public function process()
    {
        ...
        if (Phpfox::getLib('setting')->getFromServerConfigFile('core.mail_from_name')) {
            unset($aValidation['mail_from_name']);
        }
        if (Phpfox::getLib('setting')->getFromServerConfigFile('core.email_from_email')) {
        ...
        if ($sGroupId == 'mail' && $this->request()->get('test')) {
            ...
            if (isset($aVals['email_send_test']) && (!$aValidation || ($aValidation && !empty($aVals['value']) && $oValidator->isValid($aVals['value'])))) {
                if (filter_var($aVals['email_send_test'], FILTER_VALIDATE_EMAIL)) {
                    ...
                    if (!Phpfox::getLib('setting')->getFromServerConfigFile('core.method')) {
        ...
        foreach ($aSettings as $index => $aSetting) {
            ...
            $serverConfig = Phpfox::getLib('setting')->getFromServerConfigFile($aSetting['module_id'] . '.' . $aSetting['var_name']);
            $aSettings[$index]['is_file_config'] = ($serverConfig !== false);
        }

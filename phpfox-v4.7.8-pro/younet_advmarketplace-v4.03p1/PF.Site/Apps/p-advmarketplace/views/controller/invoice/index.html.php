...
<div class="p-advmarketplace-invoice-search-wrapper">
    <div class="p-advmarketplace-invoice-search-content">
        <form  method="GET" action="{url link='advancedmarketplace.invoice'}">
        ...
        <div class="p-advmarketplace-table-content">
            ...
            <div class="p-advmarketplace-table-row">
                ...
                <div class="item-status">
                    ...
                    <div class="item-text-info">
                        <span class="item-status-info {if $aInvoice.status == 'pending'}fw-bold{/if}">{$aInvoice.status_phrase}</span>{if $aInvoice.status === null || $aInvoice.status == 'pending'}<a href="javascript:void(0);" onclick="tb_show('{_p('advancedmarketplace_purchase_review_and_confirmation_replacement')}', $.ajaxBox('advancedmarketplace.confirmPurchase', 'height=300&width=600&listing_id={$aInvoice.listing_id}&invoice_id={$aInvoice.invoice_id}')); return false;" class="btn btn-primary btn-xs">{_p var='pay_now'}</a>{/if}
                    </div>
                </div>
            </div>
            {/foreach}

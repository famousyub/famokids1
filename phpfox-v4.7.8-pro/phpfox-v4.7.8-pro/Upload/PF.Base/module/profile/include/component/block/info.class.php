    ...
    public function process()
    {
        ...
        if ($hasAccess = Phpfox::getService('user.privacy')->hasAccess($aUser['user_id'], 'profile.basic_info')) {
            ...
            if ($aUser['user_id'] == Phpfox::getUserId()) {
                ...
                $aInfo = [_p('space_used') => ($totalUploadSpace === 0 ? _p('space_total_out_of_unlimited', ['space_total' => Phpfox_File::instance()->filesize($totalSpaceUsed)]) : _p('space_total_out_of_total', ['space_total' => Phpfox_File::instance()->filesize($totalSpaceUsed), 'total' => $totalUploadSpace]))];
            }
